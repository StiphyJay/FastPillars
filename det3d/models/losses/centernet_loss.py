...
from det3d.core.utils.center_utils import _transpose_and_gather_feat, bbox3d_overlaps_iou, bbox3d_overlaps_giou, bbox3d_overlaps_diou
...
class IouLoss(nn.Module):
  ...
  def forward(self, iou_pred, mask, ind, box_pred, box_gt):
    ...
    # target = boxes_iou3d_gpu(pred_box[mask], box_gt[mask])
    target = 2 * target - 1
    ...
    # print('code in 7')
    if torch.isnan(loss):
      print('IouLoss pred: ', pred)
      print('IouLoss target: ', target)
      exit()
    return loss
