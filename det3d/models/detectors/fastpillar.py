...
class FastPillar(SingleStageDetector):
    def __init__(
        self,
        reader,
        backbone,
        ...
        super(FastPillar, self).__init__(
    ...
    def forward(self, example, return_loss=True, **kwargs):
        ...
        preds, _ = self.bbox_head(bev_feature, backbone_features, self.test_cfg)
        if return_loss:
            return self.bbox_head.loss(example, preds, self.test_cfg)
        else:
            return self.bbox_head.predict(example, preds, self.test_cfg)

    def forward_two_stage(self, example, epoch, return_loss=True, **kwargs):
        batch_size = len(example['metadata'])
        ...
        if return_loss:
            return boxes, bev_feature, backbone_features, self.bbox_head.loss(example, preds, epoch, self.test_cfg)
        else:
