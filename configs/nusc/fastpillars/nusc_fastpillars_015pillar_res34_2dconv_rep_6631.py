...
model = dict(
    ...
    backbone=dict(
        type="RepMiddlePillarEncoder2X34_6631", ds_factor=8, #SpMiddlePillarEncoder18
        pc_range=[-54, -54, -5.0, 54, 54, 3.0], deploy=False, leakyrelu=True, use_SPD=False, atten_pool=True, use_max=True,
        pillar_cfg=dict(pool=dict(bev=0.075 * 2),
        ),
    ),
    neck=dict(
        type="RPNV2",
        ...
        num_input_features=[256, 512],   # [256, 256]
...
train_preprocessor = dict(
    ...
    db_sampler=db_sampler, #db_sampler
...
train_pipeline = [
    ...
    dict(type="AssignLabel_fastpillar", cfg=train_cfg["assigner"]),
    dict(type="Reformat"),
